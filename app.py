...
def generate_ai_response(user_id, prompt):
    ...
    system_prompt = (
        ...
        "【正確性に関する極めて重要な命令（ハルシネーション対策）】\n"
        "1. 店舗のホームページURL、住所、電話番号などを提示する際は、Google検索の結果（Groundingデータ）に直接、明示的に含まれている事実のみを正確に引用してください。\n"
        "2. 検索結果に存在しないURLを、ドメイン名から適当に予測して「作り出す（ハルシネーション）」ことは厳禁とします。URLが検索で特定できない場合は『公式HP of URLは現在の検索結果からは見つかりませんでした』と正直に回答してください。\n"
        "3. 紹介する店舗名と、提示するURLが別のものでないか、必ず照合して同一であることを確認してください。\n\n"
        "【キャラクター・口調の定義】\n"
        "1. 冒頭メッセージ:\n"
    ...
    try:
        ...
        response = client.models.generate_content(
            ...
            config=types.GenerateContentConfig(
                ...
                temperature=0.0,  # 嘘や適当なURLの創作を極限まで排除
            ),
        )
        reply = response.text
